...
class NMT(nn.Module):
    ...
    def encode(
        ...
        output, rest = self.encoder(pack_padded_sequence(X, lengths=source_lengths))
        output = pad_packed_sequence(output)
        output = output[0]
        enc_hiddens = torch.permute(output,(1,0,2))
        h,c = rest
        hh = torch.cat([h[0],h[1]],1)
        cc = torch.cat([c[0],c[1]],1)
        dec_init_state = (self.h_projection(hh),self.c_projection(cc))
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        Y_t = torch.split(Y,split_size_or_sections=1,dim=0)
        for t,wrd in enumerate(Y_t):
            y = torch.squeeze(wrd,dim=0)
            Ybar_t = torch.cat((y,o_prev),1)
            dec_state,o_new,_ = self.step(Ybar_t,dec_state,enc_hiddens,enc_hiddens_proj,enc_masks)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        dec_state = dec_state_new
        dec_hidden_unsq = torch.unsqueeze(dec_hidden,dim=2)
        e_t = torch.bmm(enc_hiddens_proj,dec_hidden_unsq)
        ...
        alp = torch.unsqueeze(alpha_t,dim=1)
        a_t = torch.bmm(alp,enc_hiddens)
